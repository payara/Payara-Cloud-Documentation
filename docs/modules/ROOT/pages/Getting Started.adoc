...
The application name is important as it is also the DNS name assigned to the container running your application and can be used to access the application directly from another application in the same namespace. The Runtime Type will select Payara 5, JDK 11 if application is of Jakarta EE 8 or less version and Payara 6, JDK 11 if application is of Jakarta EE 9 or higher version.
...
You can change your runtime type to Payara 6, JDK 11 to use Payara 6 enterprise if application is of Jakarta EE 9 or higher version. The default runtime type is Payara 5, JDK 11 to use Payara 5 enterprise if application is of Jakarta EE 8 or less version.
