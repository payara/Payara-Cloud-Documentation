...
Click on the btn:[Edit Configuration] menu item under the Application Actions page to enter the configuration page.
...
Before deploying your application, you can configure the following:

[NOTE]
====
Configuration for *Runtime Type* and *Runtime Size* is optional. If you do not set these manually, the platform will automatically select an appropriate runtime and size based on an inspection of your uploaded artifact.
====
...
[#java-runtime]
**Java Runtime** - Select the JDK on which your application will run:
...
**Database Connection** (if applicable) - If your application connects to a datasource, provide the following details:

* JDBC URL: The connection string for your database.
* Username and Password: (Optional) Credentials for database authentication. If not provided here, they may be read from the JDBC URL string itself, depending on your JDBC driver.
* Datasource class name: (Optional) This is almost always auto-detected from the JDBC URL.

[NOTE]

For most of the configuration, if you do not explicitly configure, default options will be selected for your application.
...
* **Jakarta EE/MicroProfile applications**: Looks for a `persistence.xml` file that uses the Jakarta EE https://nofluffjuststuff.com/blog/arun_gupta/2014/07/defaults_in_java_ee_7_tech_tip_37_#:~:text=nobeans%2Dxml.-,Default%20data%20source,-%3A%20A%C2%A0Java%20EE[default datasource] `java:comp/DefaultDataSource`. If detected, you can specify database source configuration parameters for the referenced JNDI name.
...
If detected, {productName} makes the necessary minimal configuration parameters for the default datasource available in the Console.
