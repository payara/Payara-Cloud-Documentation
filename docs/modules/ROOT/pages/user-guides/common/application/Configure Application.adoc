...
{productName} supports MicroProfile Config, Spring Boot and Quarkus configuration properties for managing your application configuration.
...
[NOTE]
====
When {productName} detects that configuration is required before deployment (such as when a `persistence.xml` file is found), the application status displays as "unconfigured" and the Deploy action will be unavailable until you provide the necessary configuration.
====

[#deployment-configuration]
== Deployment Configuration

Before deploying your application, configure the deployment settings to ensure your application runs in the correct environment with correct resources.

[NOTE]
====
Configuration for *Runtime Type*, *Java Runtime*, and *Runtime Size* is optional. If not specified, {productName} will automatically select matching values based on your application artifact.
====
...
[checklist]
* ❶ Scalability Type: Choose from three scalability types for your application.
* ❷ Runtime Type: The underlying combination of Java and Jakarta EE, Spring Boot, or Quarkus versions.
...
=== Scalability Types
{productName} supports three scalability types: Rolling Upgrade, Singleton, and Horizontal Scaling.
...
{productName} determines the readiness of an application by calling the MicroProfile Health (for Jakarta EE/Java EE and Quarkus apps) and Spring Actuator endpoint if bundled.
...
{productName} will create and destroy the replicas based on application load.
You can also enable Data Grid Mode to use https://www.payara.fish/products/features-catalog/domain-data-grid/[Payara Data Grid] for the application.
...
The runtime size specifies the amount of resources to allocate for this instance and consumes from the included vCPU of your subscription. The resource allocation is as follows:
...
This feature allows your application to automatically scale down to zero instances when idle.
...
[#application-wake-up-behavior]
==== Application Wake-Up Behavior

When an application has been scaled to zero, the first incoming request triggers the application to start up.
The request is held during this startup period until the application becomes ready to handle it.

[WARNING]
====
Clients should configure an appropriate read timeout to accommodate the application startup time.
Browser clients typically handle this automatically, but programmatic clients (HTTP clients, API clients, etc.) must set a timeout that exceeds the expected application startup duration.
====

During the wake-up process, the following behaviors occur:

* **307 Temporary Redirect**: Once the application starts up, {productName} returns a `307 Temporary Redirect` status code. Clients must properly handle this redirect by:
  ** Repeating the request to the new location
  ** Preserving the original HTTP method (POST, PUT, etc.)
  ** Including the original request body

* **503 Service Unavailable**: In rare cases where the application fails to wake up, {productName} returns a `503 Service Unavailable` status code with an HTML payload describing the failure. Clients must:
  ** Detect the 503 response
  ** Implement retry logic with exponential backoff
  ** Handle the HTML response appropriately (not parse as JSON/XML)
...
* **Azul Zulu** (default): Standard OpenJDK distribution
...
[NOTE]
====
Azul Zing is part of the xref:user-guides/common/application/integrations.adoc#_azul_platform_prime[Azul Platform Prime] offering and is only available if you have opted in during contract negotiations.
It provides performance improvements and cost savings for demanding production workloads through more efficient CPU utilization and faster execution.
====
...
|Minimal
|The instance immediately shuts down when new instance reached ready state.
...
The context root is the URL path where your application is accessible.
...
The configuration mechanism depends on your application type:
...
[checklist]
* ❶ Restrict Fields: Mark fields as restricted for confidentiality. Values of restricted fields display as * characters
* ❷ Add Config Property: Clicking this button opens the form to add a new configuration property for your application
...
This keeps your application's source configuration files unchanged while still allowing you to override or add configuration values through the platform interface.
====
...
Consult the https://quarkus.io/guides/all-config[Quarkus Configuration Reference] to determine a property's configuration phase. Properties marked with a lock icon or labeled as "build time" cannot be changed after the application is built.
...
quarkus.datasource.username=${DATABASE_USERNAME:myuser}
quarkus.datasource.password=${DATABASE_PASSWORD:mypassword}
...
You can select any number of fields to set as restricted. After clicking save, the restricted fields display as follows.
...
If you are using plain JDBC connections instead of the JPA functionality of Jakarta EE, use the @DataSourceDefinition:
...
The MicroProfile Config property MPCONFIG externalizes the properties needed for the connection. This allows you to set different values for different environments and namespaces in {productName}.
...
[NOTE]
====
If you are using plain JDBC connections instead of the JPA functionality of Jakarta EE, define the MicroProfile configuration keys manually in {productName}. Click 'Add Configuration Key' and enter the property value. In both cases, make sure the appropriate database driver is included in the WAR file you deploy.
====
...
----
# Primary datasource
spring.datasource.primary.url=${DATABASE_URL:jdbc:postgresql://localhost:5432/primary_db}
spring.datasource.primary.username=${DATABASE_USERNAME:primary_user}
spring.datasource.primary.password=${DATABASE_PASSWORD:primary_password}

# Secondary datasource
spring.datasource.secondary.url=${DATABASE_URL:jdbc:mysql://localhost:3306/secondary_db}
spring.datasource.secondary.username=${DATABASE_USERNAME:secondary_user}
spring.datasource.secondary.password=${DATABASE_PASSWORD:secondary_password}
----
...
quarkus.datasource.username=${DATABASE_USERNAME:myuser}
quarkus.datasource.password=${DATABASE_PASSWORD:mypassword}
quarkus.datasource.jdbc.url=${DATABASE_URL:jdbc:mysql://localhost:3306/mydb}
...
# Default datasource
quarkus.datasource.db-kind=postgresql
quarkus.datasource.jdbc.url=${DATABASE_URL:jdbc:postgresql://localhost:5432/primary_db}
quarkus.datasource.username=${DATABASE_USERNAME:primary_user}
quarkus.datasource.password=${DATABASE_PASSWORD:primary_password}

# Named datasource
quarkus.datasource.secondary.db-kind=mysql
quarkus.datasource.secondary.jdbc.url=${DATABASE_URL:jdbc:mysql://localhost:3306/secondary_db}
quarkus.datasource.secondary.username=${DATABASE_USERNAME:secondary_user}
quarkus.datasource.secondary.password=${DATABASE_PASSWORD:secondary_password}
----
...
To provision a database for optimal performance, provision it on Microsoft Azure in the same region as the namespace.
...
When satisfied with your configuration, click Save to return to the application details screen.
