...
If detected, it makes the necessary minimal configuration parameters for the default datasource available in the {productName} Console.
...
[NOTE]
ifeval::["{page-site-edition}" == "qube"]
You must define the MicroProfile xref:user-guides/common/application/Configure Application.adoc#_adding_configuration_properties[configuration keys] passed to MPCONFIG manually in {productName}. You can then set values for those keys to be passed to the datasource definition parameters.
endif::[]
...
For Quarkus applications deployed to {productName}, database connections are configured using standard Quarkus properties in your `application.properties` file.
