...
[#_initiating_an_additional_subscription]
=  Initiating an Additional Subscription

All users of {productName} have the ability to initiate additional subscriptions, provided they have access to the necessary payment information.
...
[#_signing_up_with_existing_payment_details]
== Signing Up with Existing Payment Details

Selecting a plan is the first step, similar to the regular signup process.
Note that usually, you are not eligible for a trial subscription for an additional plan, as {productName} allows only one trial per user.
...
You also have the option to subscribe to {productName} product updates via a newsletter.
