...
= {productName} Pricing and Subscriptions
...
{productName} billing is based on the total vCPU (virtual CPU) resources consumed by your applications over time, measured in vCPU-minutes.
...
The basic billing unit in {productName} is vCPU-minutes, calculated as:
...
=== Free Trial
For new users, the best way to begin is with our 15-day free trial. A detailed guide to get you started is available here: xref:getting-started/paas/signup.adoc[].

=== Existing Users
If you're already using {productName}, you can easily create additional subscriptions by either xref:user-guides/paas/billing/signup/additional.adoc#_signing_up_with_existing_payment_details[reusing existing invoicing data]
or xref:user-guides/paas/billing/signup/additional.adoc#_initiating_an_additional_subscription[providing new invoicing data].

== Invoicing

{productName} issues invoices based on 30-day cycles, which include:
...
A subscription can be cancelled at any time by following the procedure listed here: xref:getting-started/paas/cancel-subscription.adoc[].

== Payment Options

{productName} accepts both credit and debit card payments processed securely through Stripe.
endif::[]
