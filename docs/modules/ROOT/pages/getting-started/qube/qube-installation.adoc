...
.Prerequisites to install additional regions in an existing Qube deployment:

* Ensure there is existing Qube deployment in the same AWS account
* Use same Stack name as existing deployment
* URL: $existing-url/install-qube/region

image::docs:ROOT:qube/installation/qube-install-7.png[Install additional region,width=624,height=257,link="{imagesdir}/qube/installation/qube-install-7.png", window="_blank"]

1. Set your Region ID (typically matches stack ID) and Region Description
+
...
2. Click **Generate installation data** to download the zip file
