...
{productName} uses a role-based access control system, where each role is associated with a specific set of permissions.
This ensures that users have the appropriate level of access to manage and interact with the {productName} environment based on their responsibilities.
...
* No Access: By default, this role grants no access to any part of the {productName} environment. However, it can be customized on a per-namespace basis to allow specific, limited access to certain namespaces.

== Managing User Access

{productName} provides a streamlined process for managing user access and permissions within your subscription:

* Inviting Users: To grant access to your {productName} subscription, you need to invite users via email. The invitation will include a link that allows them to either create a new {productName} account or use an existing one to join the subscription.
...
{productName} offers flexibility in assigning roles at different levels or scopes, providing granular control over user access:
...
The tables below show the specific actions that each role can perform within the context of application and namespace management in {productName}.
These tables provide a clear overview of the access control mechanisms and how different roles interact with the {productName} environment.
...
These tables show the granular access control that {productName} provides, ensuring that users have the necessary permissions to perform their tasks while maintaining the security and integrity of your cloud environment.
