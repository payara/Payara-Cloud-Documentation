= Payara Qube (Managed) CLI User Guide
:man-prefix: #
:man-suffix: 1
:imagesdir: images/
...
`pcl` is command line utility for managing Payara Qube (Managed) namespaces and applications.
...
`pcl` uses OAuth Device Authentication flow to establish its identity to Payara Qube (Managed).
It will generate a confirmation code, which you use to log in to Payara Qube (Managed) web interface via browser.
...
You can override this to target the management endpoint for your Payara Qube (Managed) environment.
...
Payara Qube (Managed) can extend to support more regions and providers.
...
Payara Qube (Managed) will keep retrying deployment for up to 10 minutes before it pronounces deployment failed.
