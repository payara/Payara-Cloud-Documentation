...
`qube` is command-line utility for managing Payara Qube namespaces and applications.
It is distributed as executable jar file.
It requires Java Runtime, version 8 or newer.

=== Documentation Structure

User Guide demonstrates the concept and usage of `qube` mainly through examples.
...
<dependency>
  ...
  <artifactId>qube-cli</artifactId>
...
* link:{release-repo}/fish/payara/qube/qube-cli/{release-version}.zip[Full Distribution (.zip)]
* link:{release-repo}/fish/payara/qube/qube-cli/{release-version}.jar[Executable JAR only (.jar)]

// === Help
// === Output options
== Logging in

`qube` uses OAuth Device Authentication flow to establish its identity to Payara Qube.
It will generate a confirmation code, which you use to log in to Payara Qube web interface via browser.
...
WARNING: Only confirm this page if you knowingly initiated login flow via `qube login`.
Payara Qube will never ask you to authenticate via this flow via any other channel or under any other circumstance, such as solving a support issue.

If you are logging in for the first time the authentication service will ask you to authorize Payara Qube CLI to perform application management tasks with your identity:
...
The token does not have any expiration time, so you will not need to log in again for when using `qube`.
Tokens can be invalidated remotely via User Preferences screen in Payara Qube Web UI.

=== Non-interactive usage

Login only works for interactive sessions.
In order to be able to perform `qube` commands in non-interactive environments such as CI `qube login` offers option `--print-token`.

[listing,subs=+quotes]
----
*qube login --print-token*
_[INFO] In order to log in follow following link: https://login.payara.cloud/activate_
_[INFO] Your confirmation code is CVJF-XKGH_
_[SUCCESS] Token for environment-based login:_
QUBE_AUTH_TOKEN=eyJyZWZyZXNoX3Rva2VuIjoiWktMTFVQaUVIeEZ3c2JfOGNXSzFILUdnaUR1OVlXZUdDR3h3TUVFUlVrM3VLIn0=
----
...
Set secret `QUBE_AUTH_TOKEN` for the repository to `eyJyZWZyZXNoX3Rva2VuIjoiWktMTFVQaUVIeEZ3c2JfOGNXSzFILUdnaUR1OVlXZUdDR3h3TUVFUlVrM3VLIn0=`.
...
----
    - name: Deploy new version
      env:
        QUBE_AUTH_TOKEN: ${{ secrets.QUBE_AUTH_TOKEN }}
      run: qube -n project-head -a main-app upload target/app.war --deploy
----

NOTE: To use qube with GitHub Actions, check out our dedicated GitHub Action at  https://github.com/payara/cloud-deploy[payara/cloud-deploy].
...
[source,bash]
----
qube login --endpoint http://custom-api.mycompany.cloud
----

[start=2]
. By using the `QUBE_ENDPOINT` Environment Variable.
+
Set the `QUBE_ENDPOINT` environment variable to override the default for all CLI commands:

[source,bash]
----
export QUBE_ENDPOINT=http://custom-api.mycompany.cloud
qube login
----

NOTE: The CLI uses the value of `QUBE_ENDPOINT` unless an `--endpoint` option is explicitly passed, which takes precedence.
...
`QUBE_NAMESPACE`: Default namespace name. Used when `-n` or `--namespace` is not specified.
Required for most commands except `login`, `list-namespaces`, and `create-namespace`.

`QUBE_SUBSCRIPTION`: Default subscription name. Used when `-s` or `--subscription` is not specified.
...
Use xref:{man-prefix}qube-list-namespaces{man-suffix}[ list-namespaces] to see namespaces available to you:
...
Creating namespace with xref:{man-prefix}qube-create-namespace{man-suffix}[create-namespace] takes exact same input as GUI -- project and stage the namespace should represent:
...
Payara Qube can extend to support more regions and providers.
...
Namespace can be deleted by using xref:{man-prefix}qube-delete-namespace{man-suffix}[delete-namespace] command:
...
A new application can be created within a namespace by uploading its binary artifact using the xref:{man-prefix}qube-upload{man-suffix}[upload] command:
...
Deployment can be either performed by the  xref:{man-prefix}qube-deploy{man-suffix}[deploy] command, or by providing option `--deploy` to the `upload` command.
...
To deploy an application as a separate step (for example after configuring it) use the xref:{man-prefix}qube-deploy{man-suffix}[ deploy] command:

include::examples/_Deployment-deployingAfterPendingChangesMovesToRunningState-01.adoc[]

Payara Qube will keep retrying deployment for up to 10 minutes before it pronounces deployment failed.
...
These both options are also available to other commands that might trigger deployment -- xref:{man-prefix}qube-upload{man-suffix}[upload] and xref:{man-prefix}qube-configure{man-suffix}[configure].
...
In order to list applications within a namespace use command xref:{man-prefix}qube-list-applications{man-suffix}[list-applications], that shows application names and their basic properties:

include::examples/_Deployment-listApplicationsShowsState-04.adoc[]

=== Stopping an Application

A running application can be stopped with xref:{man-prefix}qube-stop{man-suffix}[stop], providing the application name.
...
A stopped application can be started again by using the xref:{man-prefix}qube-start{man-suffix}[start] command.
...
The most dynamic command of cli is xref:{man-prefix}qube-configure{man-suffix}[configure].
...
Command xref:{man-prefix}qube-deploy{man-suffix}[deploy] needs to be invoked to make them live.
