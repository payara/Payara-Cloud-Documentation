...
Use xref:reference/qube-list-namespaces.adoc[ list-namespaces] to see namespaces available to you:
...
Creating namespace with xref:reference/qube-create-namespace.adoc[create-namespace] takes exact same input as GUI -- project and stage the namespace should represent:
...
Namespace can be deleted by using xref:reference/qube-delete-namespace.adoc[delete-namespace] command:
...
A new application can be created within a namespace by uploading its binary artifact using the xref:reference/qube-upload.adoc[upload] command:
...
Deployment can be either performed by the  xref:reference/qube-deploy.adoc[deploy] command, or by providing option `--deploy` to the `upload` command.
...
To deploy an application as a separate step (for example after configuring it) use the xref:reference/qube-deploy.adoc[ deploy] command:
...
These both options are also available to other commands that might trigger deployment -- xref:reference/qube-upload.adoc[upload] and xref:reference/qube-configure.adoc[configure].
...
In order to list applications within a namespace use command xref:reference/qube-list-applications.adoc[list-applications], that shows application names and their basic properties:

include::examples/_Deployment-listApplicationsShowsState-04.adoc[]

=== Stopping an Application

A running application can be stopped with xref:reference/qube-stop.adoc[stop], providing the application name.
...
A stopped application can be started again by using the xref:reference/qube-start.adoc[start] command.
...
The most dynamic command of cli is xref:reference/qube-configure.adoc[configure].
...
Command xref:reference/qube-deploy.adoc[deploy] needs to be invoked to make them live.
