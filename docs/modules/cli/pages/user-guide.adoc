...
:man-suffix: 1
:imagesdir: images/
:release-version: null object or invalid expression
:release-repo: https://nexus.payara.fish/repository/payara-artifacts

== Introduction

`qube` is command-line utility for managing Payara Cloud namespaces and applications.
...
* link:{release-repo}/fish/payara/cloud/qube/{release-version}/qube-{release-version}.zip[Full Distribution (.zip)]
* link:{release-repo}/fish/payara/cloud/qube/{release-version}/qube-{release-version}.jar[Executable JAR only (.jar)]

// === Help
// === Output options
== Logging in

`qube` uses OAuth Device Authentication flow to establish its identity to Payara Cloud.
